      ...
      <DialogContent className="max-w-md p-0 overflow-hidden bg-white rounded-3xl shadow-2xl border border-neutral-200">
          ...
          <div className="absolute top-3.5 start-3.5">
            {product.is_service ? (
              <Badge variant="outline" className="rounded-full bg-white/90 backdrop-blur-md text-neutral-800 border-neutral-200 px-3 py-1 font-bold text-xs shadow-xs">
                <TbSparkles className="h-3.5 w-3.5 text-emerald-600 mr-1 rtl:mr-0 rtl:ml-1" />
                {t('common.service')}
              </Badge>
            ) : isLow ? (
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2.5 rounded-2xl bg-neutral-50/80 p-3.5 border border-neutral-100 text-center">
            <div>
              <div className="text-[10px] uppercase font-bold text-neutral-400">{t('stock.sellPrice')}</div>
              <div className="text-base font-black text-neutral-900 mt-0.5">
                {product.sell_price.toFixed(2)} <span className="text-[10px]">{t('common.dh')}</span>
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase font-bold text-neutral-400">{t('stock.buyPrice')}</div>
              <div className="text-base font-semibold text-neutral-500 mt-0.5">
                {product.buy_price.toFixed(2)} <span className="text-[10px]">{t('common.dh')}</span>
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase font-bold text-emerald-700 flex items-center justify-center gap-0.5">
                <TbTrendingUp className="h-3 w-3 stroke-[2.5]" /> {t('stock.margin')}
              </div>
              <div className="text-base font-black text-emerald-600 mt-0.5">
                +{marginDH.toFixed(2)} <span className="text-[10px]">{t('common.dh')}</span>
              </div>
            ...
            <div className="space-y-2 text-xs border-t border-neutral-100 pt-3 text-neutral-600">
              <div className="flex items-center justify-between">
                <span className="flex items-center gap-1.5 text-neutral-400 font-medium">
                  ...
                  {t('stock.totalShelfValue')}
                </span>
                <span className="font-extrabold text-neutral-900">
                  {totalStockValue.toFixed(2)} {t('common.dh')}
                </span>
              </div>

              <div className="flex items-center justify-between">
                <span className="text-neutral-400 font-medium">{t('stock.minAlertThreshold')}</span>
                <span className="font-bold text-neutral-800">
                  {product.min_stock_level} {t('common.units')}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-neutral-400 border-t border-neutral-100 pt-3">
            ...
            {t('stock.addedOn')} {formatDate(product.created_at)}
          </div>
